...
const Spinner = () => (
  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-[#ea2e0e] mx-auto"></div>
);
...
const ShippingAddress = ({ formData, setFormData, onNext, user }) => {
  ...
  const validate = () => {
    ...
    if (!formData.phone || formData.phone.length < 9)
      newErrors.phone = "Valid phone number is required for payment.";
    setErrors(newErrors);
  ...
  const inputStyle =
    "w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#ea2e0e] transition duration-150";
  ...
  return (
    ...
    >
      <h3 className="text-xl font-bold text-gray-800 border-b pb-3 mb-4 flex items-center">
        <HiMapPin className="w-6 h-6 mr-2 text-[#ea2e0e]" />
        Shipping Details
      </h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            readOnly
            className={`${inputStyle} bg-gray-100 text-gray-600 cursor-not-allowed`}
          />
        </div>
        <div>
      ...
      <div className="flex justify-end">
        <button
          type="submit"
          className="px-6 py-2 bg-[#ea2e0e] text-white font-semibold rounded-lg hover:bg-[#c4250c] transition duration-200"
        >
...
const ShippingMethod = ({ formData, setFormData, onNext, onPrev }) => {
  const methods = [
    ...
    },
    {
      ...
      name: "Free Delivery",
      cost: 0,
      time: "Those around Kitale/Bungoma",
    },
  ];
  ...
  return (
    <div className="space-y-6 bg-white p-6 rounded-lg shadow-lg">
      <h3 className="text-xl font-bold text-gray-800 border-b pb-3 mb-4 flex items-center">
        <HiTruck className="w-6 h-6 mr-2 text-[#ea2e0e]" />
        Shipping Method
      </h3>
        ...
        {methods.map((method) => (
          ...
          >
            <div className="flex justify-between items-center">
              ...
              <span className="text-lg font-bold text-[#ea2e0e]">
      ...
      <div className="flex justify-between pt-4">
        <button
          onClick={onPrev}
          className="px-4 py-2 text-gray-600 hover:text-gray-800 flex items-center transition duration-150"
        >
...
const PaymentMethod = ({ formData, setFormData, onNext, onPrev }) => {
  ...
  return (
    <div className="space-y-6 bg-white p-6 rounded-lg shadow-lg">
      <h3 className="text-xl font-bold text-gray-800 border-b pb-3 mb-4 flex items-center">
        <HiCreditCard className="w-6 h-6 mr-2 text-[#ea2e0e]" />
        Payment Method
      </h3>
      <div className="space-y-3">
        {methods.map((method) => (
  ...
  const handlePlaceOrder = async (orderTotal, subtotal, tax) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://one-man-server.onrender.com/api/orders",
        {
          method: "POST",
          headers: {
    ...
    <div className="container mx-auto px-4 py-10 min-h-screen bg-gray-50">
      ...
      <div className="mb-10 flex justify-between max-w-4xl mx-auto">
        {steps.map((step, index) => (
          ...
          >
            <div
              className={`w-10 h-10 flex items-center justify-center rounded-full text-white font-bold transition-colors duration-300 ${
                currentStep === index + 1
                  ? "bg-[#ea2e0e] shadow-lg"
                  : currentStep > index + 1
                  ? "bg-green-500"
            ...
            {index < steps.length - 1 && (
              <div
                className={`absolute top-5 right-1/2 w-full h-0.5 transform translate-x-full transition-colors duration-300 ${
      ...
      {error && (
        <div
          className="max-w-4xl mx-auto p-4 mb-6 bg-red-100 border border-red-400 text-red-700 rounded-lg"
          role="alert"
        >
